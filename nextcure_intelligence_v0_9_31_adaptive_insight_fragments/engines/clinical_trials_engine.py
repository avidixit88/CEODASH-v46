...
def _proof_burden_read(sig: ClinicalLaneSignature | None) -> str | None:
    """Q2 support: say what investors will ask, in plain language."""
    if sig is None:
        return None
    questions: list[str] = []
    if sig.patient_selection_strength in {"prominent", "present"}:
        questions.append("which patients should be selected for CDH6 and why")
    if sig.combination_strength in {"prominent", "present"}:
        questions.append("whether the benefit can stand clearly enough without being lost inside combination therapy")
    if sig.safety_strength in {"prominent", "present"}:
        questions.append("whether dosing and tolerability make the drug usable in the real treatment setting")
    if sig.response_strength in {"prominent", "present"}:
        questions.append("whether response and durability are strong enough to matter clinically")
    if not questions:
        questions.append("whether the evidence path is strong enough to separate CDH6 from broader ADC noise")
    if len(questions) == 1:
        body = questions[0]
    else:
        body = ", ".join(questions[:-1]) + f", and {questions[-1]}"
    return f"The investor questions become practical: {body}."
